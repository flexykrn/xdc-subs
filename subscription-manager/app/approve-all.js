...
const BUNDLER_KEY = process.env.DEPLOYER_PRIVATE_KEY;
...
const PM_ABI = ["function getHash(address sender,uint256 nonce,bytes32 initCodeHash,bytes32 callDataHash,bytes32 accountGasLimits,uint256 preVerificationGas,bytes32 gasFees,uint256 chainid,address paymaster) view returns (bytes32)"];

async function main() {
  const provider = new ethers.JsonRpcProvider(RPC_URL);
  const wallet = new ethers.Wallet(BUNDLER_KEY, provider);
  const ep = new ethers.Contract(ENTRYPOINT, EP_ABI, wallet);
  const pm = new ethers.Contract(SPONSOR_PAYMASTER, PM_ABI, provider);
  ...
  for (const tokenAddr of TOKENS) {
    ...
    // Build execute(approve) callData
    ...
    const initCodeHash = ethers.keccak256("0x");
    const callDataHash = ethers.keccak256(callData);
    const paymasterAndDataEmpty = "0x";
    const paymasterDataHash = ethers.keccak256(paymasterAndDataEmpty);
    
    // Compute userOp hash manually (EntryPoint v0.7 format)
    const inner = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
      ["address","uint256","bytes32","bytes32","bytes32","uint256","bytes32","bytes32"],
      [SA, nonce, initCodeHash, callDataHash, accountGasLimits, 50000, gasFees, paymasterDataHash]
    ));
    ...
    const pmHash = await pm.getHash(SA, nonce, initCodeHash, callDataHash, accountGasLimits, 50000, gasFees, 51, SPONSOR_PAYMASTER);
    const pmSignature = await wallet.signMessage(ethers.getBytes(pmHash));
    const pmAndData = ethers.concat([SPONSOR_PAYMASTER, ethers.zeroPadValue("0x249F0", 16), ethers.zeroPadValue("0xC350", 16), pmSignature]);
    
    // Sign full userOp
    const pmDataHash = ethers.keccak256(pmAndData);
    const inner2 = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
      ["address","uint256","bytes32","bytes32","bytes32","uint256","bytes32","bytes32"],
      [SA, nonce, initCodeHash, callDataHash, accountGasLimits, 50000, gasFees, pmDataHash]
    ));
    const userOpHash2 = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(
      ["bytes32","address","uint256"],
      [inner2, ENTRYPOINT, 51n]
    ));
    const signature = await wallet.signMessage(ethers.getBytes(userOpHash2));
