...
async function main() {
  ...
  // Try a simple ETH transfer instead of execute
  const callData = "0x";
  const nonce = 999; // Use a high nonce to avoid conflicts
  ...
  const pmAndData = ethers.concat([
    PAYMASTER,
    ethers.zeroPadValue("0x0186A0", 16),
    ethers.zeroPadValue("0x00", 16),
    pmSignature
  ]);
